...
# Visualization of Zeta Analysis

# our result_stylo variable contains the results of the last analysis. We can visualize them with the "plot" function. 
# The "oppose" analysis has generated a table with the most distinctive words.
# We can visualize it with the "plot" function (let's select only the 50 most distinctive words)

plot(results_stylo$table.with.all.zscores)

# we can also visualize the same table with a word cloud
library(wordcloud)
wordcloud(words = rownames(results_stylo$table.with.all.zscores), freq = results_stylo$table.with.all.zscores$zscore, min.freq = 1, max.words = 50, random.order = F, colors = brewer.pal(8, "Dark2"))
