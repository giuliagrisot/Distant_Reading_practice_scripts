...
# There are other methods to perform topic modeling: one very popular is the package mallet
...
tm_corpus <- list()
my_texts <- character()
file_list <- list.files("data/corpus", pattern = "*.txt", full.names = T)

# for the purpose of this script, we will use 10 random texts in the corpus
file_list <- sample(file_list, 5)
...
# visualize the output (let's just print the first 300 characters of the first text, with the function str_sub)

str_sub(my_texts[1], 1, 300)


# save the texts in a file
writeLines(my_texts, "data/corpus/corpus_mallet.txt")

# 2. Topic modeling with Mallet

# preparation of texts for topic model
text.instances <- mallet::mallet.import(text.array = my_texts)
