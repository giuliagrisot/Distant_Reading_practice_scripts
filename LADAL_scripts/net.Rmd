...
The example that we will be concerned with focuses on the first type of data as it is by far the most common way in which relationships are coded.
To show how to create a network, we will have a look at the network that the characters in William Shakespeare's *Romeo and Juliet* form.
...
tg %>%
   ...
   geom_edge_arc(colour= "gray",
                  lineend = "round",
                 strength = .1) +
   geom_node_point(size=log(v.size)*2, colour="gray60") +
   geom_node_text(aes(label = name), 
                  repel = TRUE, 
                  point.padding = unit(0.2, "lines"), 
                  size=sqrt(v.size), 
